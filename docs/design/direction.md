...
- **Activity**: Requests (`requests`), Jobs (`jobs`), Queues (`queues`,
  detects `queue_metrics_.*` from cboxdk/laravel-queue-metrics), Autoscale
  (`autoscale`, detects `queue_autoscale_.*` from
  cboxdk/laravel-queue-autoscale), Commands (`commands`),
...
| Queues | cboxdk/laravel-queue-metrics observable gauges: `queue_metrics_queue_depth{state=pending\|scheduled\|reserved}`, `queue_metrics_queue_oldest_job_age_seconds`, `queue_metrics_queue_throughput_per_min`, `queue_metrics_queue_failure_rate_percent`, `queue_metrics_queue_active_workers` (all by `connection`/`queue`); fleet: `queue_metrics_workers_count{state=busy\|idle}`, `queue_metrics_workers_utilization_percent{window=current\|lifetime}` |
