...
| `ignore_own_requests` | `TELEMETRY_UI_IGNORE_OWN_REQUESTS` | `true` | Asks `cboxdk/laravel-telemetry` (`Telemetry::ignorePaths()`) not to trace the dashboard's own page loads and API calls (`{path}` and `{path}/*`), so browsing the dashboard never floods the app's request data. Registered when the telemetry manager resolves; no effect when mounted at the site root. |
