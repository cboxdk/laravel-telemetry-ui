 ...
 */
final class AutoscaleCluster extends Panel
{
    ...
    public function data(): array
    {
        ...
        try {
            foreach (self::SERIES as $metric => [$label, $color]) {
                $range = $this->metrics()->queryRange($this->metric($metric)->maxBy(), $start, $end);
            ...
            $managers = $this->total($this->metric('queue_autoscale_cluster_managers')->maxBy());
            $utilization = $this->total($this->metric('queue_autoscale_cluster_utilization_percent')->maxBy());
            $hosts = $this->total($this->metric('queue_autoscale_cluster_recommended_hosts')->maxBy());
